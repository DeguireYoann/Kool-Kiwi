                  ...
                  <form class="mt-4 border-t border-gray-200">
                    ...
                    <ul role="list" class="px-2 py-3 font-medium text-gray-900">
                      <li v-for="category in subCategories" :key="category.name">
                        <NuxtLink :to="category.href" class="block px-2 py-3">{{ category.name }}</NuxtLink>
                      </li>
                    </ul>
                  ...
                  <MenuItems class="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div class="py-1">
                      <MenuItem v-for="option in sortOptions" :key="option.name" v-slot="{ active }">
                        <NuxtLink :to="option.href" :class="[option.current ? 'font-medium text-gray-900' : 'text-gray-500', active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm']">{{ option.name }}</NuxtLink>
                      </MenuItem>
                    </div>
                  </MenuItems>
                </transition>
            ...
            <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
              ...
              <form class="hidden lg:block">
                ...
                <ul role="list" class="space-y-4 border-b border-gray-200 pb-6 text-sm font-medium text-gray-900">
                  <li v-for="category in subCategories" :key="category.name">
                    <NuxtLink :to="category.href">{{ category.name }}</NuxtLink>
                  </li>
                </ul>
